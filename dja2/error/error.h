...
namespace dja {
	...
	public:
		...
		/// \brief Prints the exception to the standard error stream (cerr), including its messageand stack trace if any.
		virtual void dump() const;

		/// \brief Prints the exception to the given output stream, including its messageand stack trace if any.
		virtual void dump(std::ostream& out) const;

		/// \brief Returns what kind of exception this is.
		///
		/// \attention In general this returns "error", but in some cases we catch other kinds of exceptions,
		/// and wrap them up as Exceptions.
		/// In such cases, the kind will be "int" or "string" etc.
		...
		/// based on when/why it was thrown, platform incompatibilities, and other issues.
		/// use hasStackTrace to check if a given exception's stack trace is populated.
	...
	private:
		std::string kind_, msg_, stack_trace_;
	};

	/// \brief Prints the error exception to the given output stream.
	std::ostream& operator <<(std::ostream& out, const Exception& ex);

	/// \brief Signals an error condition in a program by throwing an Exception with the specified message.

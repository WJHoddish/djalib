...
#ifndef DJA_COUNTED_PTR_H_
#define DJA_COUNTED_PTR_H_
...
class counted_ptr {
 public:
  ...
  counted_ptr(T* ptr) : ptr_(ptr), cnt_(new int(1)) {}
  ...
  counted_ptr<T>& operator*() { return *this; }
  ...
  const T* raw_ptr() const { return ptr_; }
 ...
 private:
  /// \brief Eliminate this instance, release the memory if this is the last
  /// reference.
  void destroy() {
    if (ptr_ != nullptr) {
      delete ptr_;
  ...
  int* cnt_;  ///< \brief the counter for the reference count mechanism, be
              ///< easily handled during copying
};
}  // namespace dja
